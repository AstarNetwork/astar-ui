<template>
  <div class="mykit-preview">
    ...
    <div class="preview-bottom">
      <span name="Code" @click="showSourceCode">Code</span>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    previewSourceCode() {
      return this.sourceCode.replace(/'\.\.\/\.\.\/index'/g, `'ui-kit'`);
    },
  },
  async mounted() {
    if (this.compName && this.demoName) {
      if (isDev) {
        ...
        this.sourceCode = await fetch(`${isDev ? '' : '/Astar-UI'}/packages/${this.compName}/docs/${this.demoName}.vue`).then((res) => res.text());
      }
    }
    await this.$nextTick();
    Prism.highlightAll();
